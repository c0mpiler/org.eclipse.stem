...
class CTDLGenerator implements IGenerator {
	...
	{
		if (modelGenClass !== null) {
			return modelGenClass.genModel.getImportedName(fqn);
		} else {
			return fqn;
		}
	}
...
//		return ;
	...
	def dispatch compile(TransitionBlock t) {
		...
		var labelClass = ctd.metamodel.model.compartments.class_.instanceTypeName
		if (labelClass === null) {
			labelClass = ctd.metamodel.model.compartments.class_.name
		} else {
			labelClass = addImport(labelClass);
		}
