...
public class ModelCompartmentEditorDialog extends Dialog
{
	...
	protected Compartment compartment, originalCompartment;
	protected CompartmentGroup compartmentGroup;
	
	protected String errorMessage = null;
	
	public ModelCompartmentEditorDialog(Shell parentShell, int style, CompartmentGroup compartmentGroup, Compartment compartment, Compartment originalCompartment) 
	{
		super(parentShell);
		this.compartment = compartment;
		this.originalCompartment = originalCompartment;
	...
	public void create()
	{
		...
		nameText.setText(WizardHelper.safeGet(compartment.getName()));
		
		EDataType selectedDataType = compartment.getDataType();
		if (selectedDataType == null) {
			selectedDataType = WizardHelper.getDefaultCompartmentDataTypes()[0];
		}
		
		dataTypeComboViewer.setSelection(new StructuredSelection(selectedDataType));
		...
		compartmentTypeComboViewer.setSelection(new StructuredSelection(selectedCompartmentType));
	}
	...
	private void createCompartmentTypeList(Composite container)
	{
		...
		compartmentTypeComboViewer.setContentProvider(ArrayContentProvider.getInstance());
		compartmentTypeComboViewer.setLabelProvider(new EMFLabelProvider());
	...
	private void createDataTypeList(Composite container)
	{
		...
		dataTypeComboViewer.setLabelProvider(new EMFLabelProvider());
		
		dataTypeComboViewer.setInput(WizardHelper.getDefaultCompartmentDataTypes());
		
		dataTypeComboViewer.getCombo().setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
	}
	...
	protected boolean validate()
	{
		...
		if (hasNameConflict()) {
			errorMessage = "A compartment named \""+ nameText.getText() +"\" already exists in this compartment group.";
			return false;
		}
	...
	{
		for (Compartment compartment : WizardHelper.getAllCompartmentsForGroup(compartmentGroup)) {
			if (compartment != originalCompartment && compartment.getName().equals(nameText.getText())) {
				return true;
			}
		}
		return false;
	...
	protected void okPressed() 
	{
		...
		if (validate()) {
			...
			compartment.setType((CompartmentType)((StructuredSelection)compartmentTypeComboViewer.getSelection()).getFirstElement());
			compartment.setDataType((EDataType)((StructuredSelection)dataTypeComboViewer.getSelection()).getFirstElement());
			super.okPressed();
		} else {
