		...
		assert(npts==r.length);
			...
			for(int j = 0; j < npts-1; j++){
				...
				if((sNorm<=maxSthreshold)&&(sNorm>=minSthreshold)&&(iNorm>=minIthreshold)&&(rNorm<=maxRthreshold)) {
						...
						xList1.add(Double.valueOf(xValue) );
						yList1.add(Double.valueOf(yValue));
				} else {
					// **** for plotting ******
			...
			if(xList1.size() > 2) {
				...
				for (int i = 0; i< xList1.size(); i ++) {
					...
					fitList1.add(Double.valueOf(val));
				}
			} // if fit valid
			else {
				rejectCount += 1.0;
			...
			for(int j = 0; j < npts-1; j++){
				...
				if((sNorm<=maxSthreshold)&&(sNorm>=minSthreshold)&&(eNorm>=minEthreshold) ) {
					...
					xList2.add(Double.valueOf(xValue));
					yList2.add(Double.valueOf(yValue));
				} else {
					// **** for plotting ******
					// here we define E*
					// NOT Selected = 0.0
			...
			if(xList2.size() > 2) {
				...
				for (int i = 0; i< xList2.size(); i ++) {
					...
					fitList2.add(Double.valueOf(val));
				}
			}// if fit valid
			else {
				rejectCount += 1.0;
			...
			for(int j = 0;j < npts-1;j++)
			{
				...
				if((iNorm>=minIthreshold)&&(eNorm>=minEthreshold) ) {
					xList3.add(Double.valueOf(eAvg[j] / iAvg[j] ) );
					yList3.add(Double.valueOf(dlnI[j]));
				}
			}
			// TODO:  Plot restricted data
			...
			if(xList3.size() > 2) {
				...
				for (int i = 0; i< xList3.size(); i ++) {
					...
					fitList3.add(Double.valueOf(val));
				}
			}// if fit valid
			else {
				rejectCount += 1.0;
			...
			for(int j = 0;j < npts-1;j++) {
				...
				if((iNorm>=minIthreshold)&&(rNorm>=minRthreshold)&&(rNorm<=maxRthreshold) ) {
					xList4.add(Double.valueOf(iAvg[j] / rAvg[j] ) );
					yList4.add(Double.valueOf(dlnR[j]));
				}
			}
			// TODO:  Plot restricted data
			...
			if(xList4.size() > 2) {
				...
				for (int i = 0; i< xList4.size(); i ++) {
					...
					fitList4.add(Double.valueOf(val));
				}
			}// if fit valid
			else {
				rejectCount += 1.0;
