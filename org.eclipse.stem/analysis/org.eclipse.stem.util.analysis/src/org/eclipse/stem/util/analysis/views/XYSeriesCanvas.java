...
public class XYSeriesCanvas extends Canvas {
		...
		for(int i = 0; i <= 1; i ++) {
			...
			if (doubleValuesX.length > 0) {
				...
				for (int cycleNumber = 0; cycleNumber < doubleValuesX.length; cycleNumber++) {
					Double valueX = Double.valueOf(doubleValuesX[cycleNumber]);
					Double valueY = Double.valueOf(doubleValuesY[cycleNumber]);
	...
	private void clearData() {
		...
		while((iter!=null)&&(iter.hasNext())) {
			...
			series.addValue(Double.valueOf(0.0), Double.valueOf(0.0));
		}
...
//		draw();
	...
	protected class DataXYSeries 
	{
		...
		public DataXYSeries(String propertyName) {
			...
			yValues.add(Double.valueOf(0.0));
			
			xValues = new ArrayList<Double>();
			xValues.add(Double.valueOf(0.0));
