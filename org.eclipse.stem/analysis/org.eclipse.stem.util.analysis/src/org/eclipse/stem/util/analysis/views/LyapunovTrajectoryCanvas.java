...
public class LyapunovTrajectoryCanvas extends Canvas {
			...
			if (trajectory.length > 0) {
				...
				for (int cycleNumber = 0; cycleNumber < trajectory.length; cycleNumber++) {
					this.relativeValuesX.get(i).add(
							Double.valueOf(trajectory[cycleNumber].getXValue()));
					this.relativeValuesY.get(i).add(
							Double.valueOf(trajectory[cycleNumber].getYValue()));
				} // for cycleNumber
			} else {
				resetData();
			}
	...
	protected void resetData() {
		...
		clearData();
		for (int i = 0; i < relativeValuesX.size(); i++) {
			relativeValuesX.get(i).add(Double.valueOf(1.0));
			relativeValuesY.get(i).add(Double.valueOf(0.0));
		}
	}
