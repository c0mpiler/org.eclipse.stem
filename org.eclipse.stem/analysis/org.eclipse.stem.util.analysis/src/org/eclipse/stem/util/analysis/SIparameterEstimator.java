		...
		assert(npts==i.length);
			...
			for(int j = 0; j < npts-1; j++){
				...
				if((sNorm<=maxSthreshold)&&(sNorm>=minSthreshold)&&(iNorm>=minIthreshold) ) {
					...
					xList1.add(Double.valueOf( numer ) );
					yList1.add(Double.valueOf( (pAvg[j]/iAvg[j]) * dlnS[j]  ));
				}else {
					// **** for plotting ******
			...
			for(int j = 0; j < npts-1; j++){
				...
				double iNorm = iAvg[j]/iMax;
				if((sNorm<=maxSthreshold)&&(sNorm>=minSthreshold)&&(iNorm>=minIthreshold) ) {
					xList2.add(Double.valueOf( ( pAvg[j] / sAvg[j]) ));
					yList2.add(Double.valueOf((pAvg[j]/sAvg[j]) *dlnI[j]));
				}// if data in range
			}
