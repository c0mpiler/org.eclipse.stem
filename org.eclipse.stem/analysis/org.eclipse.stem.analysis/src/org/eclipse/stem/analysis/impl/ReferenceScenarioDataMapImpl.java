			...
			for (Entry<String,List<String>> entry : data.entrySet()) {
				...
				if(valueList.size()>=1) {
					...
					try {
						...
						double val = (Double.valueOf(first)).doubleValue();
					...
					for (int i = 0; i < valueList.size(); i ++) {
						...
						if(isData) {
							...
							double oldVal = (Double.valueOf(aggregatedList.get(i))).doubleValue();
		...
		public void findMaxValues() {
					...
					if(this.dataMap.getS_Keys() != null) {
						for (int i =0; i < S_keys.length; i ++) {
							...
							if(dataList != null) {
								maxS = Double.valueOf(dataList.get(0)).doubleValue();
		            			for(int j = 0; j < dataList.size(); j ++) {
		            				double val = Double.valueOf(dataList.get(j)).doubleValue();
		            				if (val > maxS) {
		            					maxS = val;
		            				}
		            			}
					...
					if(this.dataMap.getE_Keys() != null) {
						for (int i =0; i < E_keys.length; i ++) {
							...
							if(dataList != null) {
								maxE = Double.valueOf(dataList.get(0)).doubleValue();
		            			for(int j = 0; j < dataList.size(); j ++) {
		            				double val = Double.valueOf(dataList.get(j)).doubleValue();
		            				if (val > maxE) {
		            					maxE = val;
					...
					if(this.dataMap.getI_Keys() != null) {
						for (int i =0; i < I_keys.length; i ++) {
							...
							if(dataList != null) {
								maxI = Double.valueOf(dataList.get(0)).doubleValue();
		            			for(int j = 0; j < dataList.size(); j ++) {
		            				double val = Double.valueOf(dataList.get(j)).doubleValue();
		            				if (val > maxI) {
		            					maxI = val;
					...
					if(this.dataMap.getR_Keys() != null) {
						for (int i =0; i < R_keys.length; i ++) {
							...
							if(dataList != null) {
								maxR = Double.valueOf(dataList.get(0)).doubleValue();
		            			for(int j = 0; j < dataList.size(); j ++) {
		            				double val = Double.valueOf(dataList.get(j)).doubleValue();
		            				if (val > maxR) {
		            					maxR = val;
		...
		public List<Double> getStotals() {
			...
			if(strList !=null) {
				for(int j = 0; j < strList.size(); j ++) {
					data.add(j, Double.valueOf(strList.get(j)).doubleValue());
				}
			}
			if(keys.length >=2) {
				...
				for(int i = 1; i < keys.length; i ++) {
					...
					for(int j = 0; j < strList.size(); j ++) {
	    				double val = Double.valueOf(strList.get(j)).doubleValue();
	    				val += data.get(j).doubleValue();
	    				data.set(j,Double.valueOf(val));
					}
				}
			}
			return data;
	...
	public boolean sane() {
		...
		while(locationIter!=null && locationIter.hasNext()) {
			...
			for (int t = 0; t < length; t ++) {
				double sum = 0.0;
				double s = Double.valueOf(sData.get(t)).doubleValue();
				double i = Double.valueOf(iData.get(t)).doubleValue();
				double dd = Double.valueOf(ddData.get(t)).doubleValue();
				double r = 0.0;
				double e = 0.0;
				if(eData != null) e = Double.valueOf(eData.get(t)).doubleValue();
				if(rData != null) r = Double.valueOf(rData.get(t)).doubleValue();
				if((s < 0)||(i<0)||(e<0)||(r<0)||(dd<0)) {
					System.err.println("Negative value found at time = "+t+" s="+s+" e="+e+" i="+i+" r="+r+"dd= "+dd);
					return false; //1
				}
				sum = s+e+i+r;
				if(!closeEnough(sum,Double.valueOf(popData.get(t)).doubleValue())) {
					System.err.println("Sum not correct at time = "+t+" difference is "+(sum-Double.valueOf(popData.get(t)).doubleValue()));	
					return false; // 2
				}
				// or exact equality but we need to check across different processors
//				if(sum != Double.valueOf(popData.get(t)).doubleValue()) {
//					Activator.logInformation("Sum not exactly correct at time = "+t+" difference is "+(sum-Double.valueOf(popData.get(t)).doubleValue()));
