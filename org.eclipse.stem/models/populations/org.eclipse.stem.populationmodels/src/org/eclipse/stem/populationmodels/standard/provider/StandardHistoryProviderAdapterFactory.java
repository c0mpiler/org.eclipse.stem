...
public class StandardHistoryProviderAdapterFactory extends
	...
	public Adapter createPopulationModelLabelAdapter() {
		return new PopulationModelLabelHistoryProvider();
	...
	/**
	 * This class is a History Provider for PopulationModelLabel instances. It
	 * provides a collection of historic values of a PopulationModelLabel
	 */
	private static class PopulationModelLabelHistoryProvider extends
			HistoryProviderAdapter implements HistoryProvider {

		long cycle = 0L;
		final STEMTime startTime = ModelFactory.eINSTANCE.createSTEMTime();

		final Map<STEMTime, PopulationModelLabel> history = Collections.synchronizedMap(new TreeMap<STEMTime, PopulationModelLabel>());
				...
				case GraphPackage.DYNAMIC_LABEL__NEXT_VALUE:
					...
					if (msg.getNewValue() != null) {
						...
						synchronized (history) {
							...
							final PopulationModelLabel dml = (PopulationModelLabel)EcoreUtil
									.copy((PopulationModelLabel) getTarget());
							// When you copy an EObject, the references are
							// copied as well this means that the Node that
							// referenced the original PopulationModelLabel will
	...
	} // PopulationModelLabelHistoryProvider
