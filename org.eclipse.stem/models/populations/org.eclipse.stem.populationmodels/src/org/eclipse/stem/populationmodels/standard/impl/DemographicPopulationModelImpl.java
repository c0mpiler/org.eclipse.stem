...
public class DemographicPopulationModelImpl extends StandardPopulationModelImpl implements DemographicPopulationModel {
	...
	public EList<PopulationGroup> getPopulationGroups() {
		if (populationGroups == null) {
			populationGroups = new EDataTypeUniqueEList<PopulationGroup>(PopulationGroup.class, this, StandardPackage.DEMOGRAPHIC_POPULATION_MODEL__POPULATION_GROUPS);
			//this was broken....   new BasicInternalEList<PopulationGroup>(PopulationGroup.class);
	...
	public void decorateGraph(STEMTime time) throws ScenarioInitializationException{
		...
		for (final Iterator<PopulationLabel> populationLabelIter = getPopulationLabels(
			...
			if(this.getPopulationIdentifier().equals(populationLabel.getPopulationIdentifier())) {
				...
				for(PopulationGroup group:this.getPopulationGroups()) {
					final PopulationModelLabel pl = createPopulationModelLabel(group.getIdentifier());
					URI unique = STEMURI.createURI(Label.URI_TYPE_LABEL_SEGMENT + "/"+populationLabel.getNode().getURI().lastSegment()+"/"+this.getName()+"/"+group.getIdentifier()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
					pl.setURI(unique);
