	...
	public static class PopulationModelLabelValueRelativeValueProvider extends
		...
		public List<IItemPropertyDescriptor> getProperties() {
			...
			for (IItemPropertyDescriptor descriptor : properties) {
				...
				if(feature instanceof EReference) continue; // skip arrivals etc.
...
//				} // if has relative value
		...
		public double getRelativeValue(final EStructuralFeature feature) {
			if(feature==null) return 0.0;
			final StandardPopulationModelLabelValue popv = (StandardPopulationModelLabelValue) getTarget();
			Object obj = null;
			if(popv!=null) obj = popv.eGet(feature);
			double retValue = 0.0;
			if(obj != null) {
				double count = ((Double) obj).doubleValue();
			...
			// JHK this could be > 1 but that is correct, relative value can be twice the reference value.
			// plotting routines should handle that case.
			return retValue;
		} // getRelativeValue
