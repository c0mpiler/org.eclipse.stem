...
public class DiseaseModelModelSpecification extends ModelSpecification
	...
	protected Model createIdentifiableInstance() {
		...
		try {
			File popModelFile = new File(getIdentifiableProjectURI(populationModelDC).path().replace("/resource/", "../../../../org.eclipse.stem.data/population/"));
			
			
			retValue
					.getModels()
					.add(
							(Model) Utility.getIdentifiableFromStream(getIdentifiableProjectURI(populationModelDC), 
									new FileInputStream(popModelFile), Collections.<String, Object>emptyMap(), false));
