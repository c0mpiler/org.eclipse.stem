...
public class Gaussian3ForcingDiseaseModelImpl extends Gaussian2ForcingDiseaseModelImpl implements Gaussian3ForcingDiseaseModel {
	...
	public void calculateDeltas(STEMTime time, double t, long timeDelta, EList<DynamicLabel> labels)  {
		final double adjustedTransmissionRate = getAdjustedTransmissionRate(getTransmissionRate(),timeDelta);
		final double adjustedTransmissionRate2 = getAdjustedTransmissionRate(getTransmissionRate2(),timeDelta);
		...
		for(int _i=0;_i<labels.size();++_i) {
			...
			case H0:
				f1 = adjustedTransmissionRate  * (getModulationFloor() + (1-getModulationFloor())*Math.exp(-(Math.pow(fday,2))/(2*sigma2)));
				break;	
			case H1:
				if(gaussian == 0) { // A
					f1 = adjustedTransmissionRate * (getModulationFloor() + (1-getModulationFloor())*Math.exp(-(Math.pow(fday,2))/(2*sigma2)));
				} else if(gaussian == 1) {
					f1 =adjustedTransmissionRate2 * (getModulationFloor() + (1-getModulationFloor())*Math.exp(-(Math.pow(fday,2))/(2*sigma2)));
				}
				break;	
			case H2:
				if(gaussian == 0) {
					...
					f1 = adjustedTransmissionRate2  * (getModulationFloor_2() + (1-getModulationFloor_2())*Math.exp(-(Math.pow(fday,2))/(2*sigma2_2)));
				}
				break;
			...
			if(t % modulationPeriod < WINDOWSIZE || t % modulationPeriod > modulationPeriod-WINDOWSIZE) {
				...
				case H0:
					f2 = adjustedTransmissionRate * (getModulationFloor() + (1-getModulationFloor())*Math.exp(-(Math.pow(fday-1,2))/(2*sigma2)));
					f3 = adjustedTransmissionRate  * (getModulationFloor() + (1-getModulationFloor())*Math.exp(-(Math.pow(fday+1,2))/(2*sigma2)));
					break;	
				case H1:
					if(nextGaussian == 0)
						f2 = adjustedTransmissionRate  * (getModulationFloor() + (1-getModulationFloor())*Math.exp(-(Math.pow(fday-1,2))/(2*sigma2)));
					else if(nextGaussian == 1) 
						f2 = adjustedTransmissionRate2  * (getModulationFloor() + (1-getModulationFloor())*Math.exp(-(Math.pow(fday-1,2))/(2*sigma2)));
					if(prevGaussian == 0)
						f3 = adjustedTransmissionRate  * (getModulationFloor() + (1-getModulationFloor())*Math.exp(-(Math.pow(fday+1,2))/(2*sigma2)));
					else if(prevGaussian == 1)
						f3 = adjustedTransmissionRate2  * (getModulationFloor() + (1-getModulationFloor())*Math.exp(-(Math.pow(fday+1,2))/(2*sigma2)));			
				...
				case H2:
					if(nextGaussian == 0) 
						...
						f2 = adjustedTransmissionRate2  * (getModulationFloor_2() + (1-getModulationFloor_2())*Math.exp(-(Math.pow(fday-1,2))/(2*sigma2_2)));
					if(prevGaussian == 0)
						f3 = adjustedTransmissionRate  * (getModulationFloor() + (1-getModulationFloor())*Math.exp(-(Math.pow(fday+1,2))/(2*sigma2)));
					else if(prevGaussian == 1)
						f3 = adjustedTransmissionRate2  * (getModulationFloor_2() + (1-getModulationFloor_2())*Math.exp(-(Math.pow(fday+1,2))/(2*sigma2_2)));			
					break;
