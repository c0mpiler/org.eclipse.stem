...
public class GlobalInfluenzaModelImpl extends SIRImpl implements GlobalInfluenzaModel {
	...
	public void calculateDeltas(STEMTime time, double t, long timeDelta, EList<DynamicLabel> labels)  {
		...
		double adjustedModulationPeriod = modulationPeriod * ((double)this.getTimePeriod()/(double)timeDelta);
		...
		latFactor *= Math.pow(Math.abs(Math.cos((adjustedPhase + Math.PI*t)/adjustedModulationPeriod)), seasonalModulationExponent);
...
/* NOT USED	@Override
	public void doModelSpecificAdjustments(final LabelValue state) {
			...
			double incidence = ((SILabelValue)state).getIncidence();
