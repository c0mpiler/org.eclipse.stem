...
public abstract class DynamicLabelImpl extends LabelImpl implements DynamicLabel {
	...
	public void switchToNextValue() {
		...
		// between a label and it's current and next 
		// value. If we are dealing with integration
		// labels it's more efficient to simply
		// use the set(...) method on the 
		...
		if(!(this instanceof IntegrationLabel)) {
			...
			IntegrationLabelValue currentIVal = (IntegrationLabelValue)getCurrentValue();
			currentIVal.set(((IntegrationLabelValue)getNextValue()));
			
			// Workaround to get the time series plots which depends on notification on the SET_NEXT_VALUE
			// to keep the history
			setNextValue(getNextValue());
			setNextValueValid(false);
			for(EAttribute ea:getNextValue().eClass().getEAllAttributes()) 
				if(((IntegrationLabelValue)getNextValue()).eGetDouble(ea.getFeatureID()) < 0)
