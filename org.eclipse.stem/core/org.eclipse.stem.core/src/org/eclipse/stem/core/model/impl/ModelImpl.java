...
public class ModelImpl extends IdentifiableImpl implements Model {
	...
	/**
	 * This method is identical to getCanonicalGraph, except decorators in the model are invoked to create their labels and
	 * label values. It's used for instance when building a temporary canonical graph to allow a graph generator to do its thing.
	 * 
	 * @param uri
	 * @param filter
	 * @param time
	...
	private void invokeNodeDecorators(Graph graph, STEMTime time) throws ScenarioInitializationException {
		...
		// 261686  fix. Check for decorator dups
		
		for(int i=0;i<getNodeDecorators().size();++i) 
			for(int j=i+1;j<getNodeDecorators().size();++j)
				if(getNodeDecorators().get(i).getURI().equals(getNodeDecorators().get(j).getURI()))
					throw new ScenarioInitializationException(NLS.bind(Messages.DUPLICATE_DECORATOR, new Object[] {getNodeDecorators().get(j).getURI().toString()}), this, new Exception());
