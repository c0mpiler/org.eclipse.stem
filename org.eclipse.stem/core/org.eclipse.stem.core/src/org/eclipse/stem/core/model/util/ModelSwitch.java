...
public class ModelSwitch<T1> {
	...
	protected T1 doSwitch(int classifierID, EObject theEObject) {
		switch (classifierID) {
			...
			case ModelPackage.TRANSFORMATION_DECORATOR: {
				TransformationDecorator transformationDecorator = (TransformationDecorator)theEObject;
				T1 result = caseTransformationDecorator(transformationDecorator);
				if (result == null) result = caseNodeDecorator(transformationDecorator);
				if (result == null) result = caseDecorator(transformationDecorator);
				if (result == null) result = caseIdentifiable(transformationDecorator);
				if (result == null) result = caseCommon_Comparable(transformationDecorator);
				if (result == null) result = caseSanityChecker(transformationDecorator);
				if (result == null) result = defaultCase(theEObject);
				return result;
			}
