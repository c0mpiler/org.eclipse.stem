...
public class STEMURI {
	...
	/**
	 * Convert a file to a URI of the platform:/ type
	 * @param p The project the file is located in
	 * @param f The file
	 * @return Converted URI, or the same as input if the URI is already normalized
	 */
	
	public static URI createFromFile(File f) {
		URI uri = STEMURI.normalize(URI.createURI(f.getLocationURI().toString()));
		if(uri.scheme().equals("platform")) return uri;
