...
abstract public class NewSequentialSequencerWizard extends NewSequencerWizard {
	...
	abstract protected static class SequentialSequencerPage extends
		...
		private static final String[] cyclePeriodTimeUnitStringsPlural = {
				...
				Messages.getString("NSequencerWiz.weeks"),
				Messages.getString("NSequencerWiz.months"),
				Messages.getString("NSequencerWiz.years")};
		...
		protected boolean validatePage() {
			...
			if (retValue) {
				try {
					// Yes
					final double cycleTimeValue = Double
							.parseDouble(cyclePeriodTimeValueField.getText());
					// Is the cycle time too small. It can't be negative and can't be zero.
					if (cycleTimeValue < 1E-10) {
						// No
						retValue = false;
		...
		protected long getCycleDuration() {
			...
			try {
				final double cycleTimeValue = Double
						.parseDouble(cyclePeriodTimeString);
			...
			return new DublinCorePage() {
				...
				protected void initializeDCAttributes() {
					...
					sb.append(DublinCoreImpl
							.createISO8601DateString(((FixedSequentialSequencerPage) newIdentifiablePage)
									.getStartDate()));
					sb.append(";");
					final Date endDate = ((FixedSequentialSequencerPage) newIdentifiablePage)
							.getEndDate();
