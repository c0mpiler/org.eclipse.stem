...
public class MapCanvas 
	extends Canvas 
	implements ISelectionProvider, PaintListener, MouseWheelListener, MouseMoveListener, DisposeListener, IPropertyChangeListener
{
	...
	void draw(final GC gc, final int x, final int y, final int width,
		...
		for (final StemPolygon stempoly : polygonsToRender) {
			...
			if (identifiable instanceof Node) {
				...
				STEMColor color = colorProvider.getColor(gainFactor, useLogScaling);				
				...
				int g = (int) (alpha * c.getGreen() + (1 - alpha) * back.getGreen());
				int b = (int) (alpha * c.getBlue() + (1 - alpha) * back.getBlue());				
