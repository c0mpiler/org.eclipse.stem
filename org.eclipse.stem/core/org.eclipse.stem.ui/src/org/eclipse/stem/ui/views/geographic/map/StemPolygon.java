...
public class StemPolygon implements Comparable<StemPolygon>{
	...
	public double area = -1; // -1 means no area information available
	...
	public StemPolygon(final Segment latLongSegment, final Identifiable identifiable) {
		...
		if(identifiable != null && identifiable instanceof Region) {
			...
			EList<NodeLabel>labs = r.getLabels();
			for(int i=0;i<labs.size();++i) {
				NodeLabel lab = labs.get(i);
...
//	} // addLatLongData
	 ...
	 /**
	  * Compare to to sort polygons according to the area of the attached identifiable.
	  ...
	  */
	  public int compareTo( StemPolygon otherP ) {
	    ...
	    // Find area label of the attached identifiable
	    if(otherP.area < this.area)return BEFORE;
	    else if (otherP.area > this.area) return AFTER;
	    return BEFORE; // If equal return before to make sure they are stored propertly in the TreeSet
	  }
	/**
	 * 
	 * @param longitude
