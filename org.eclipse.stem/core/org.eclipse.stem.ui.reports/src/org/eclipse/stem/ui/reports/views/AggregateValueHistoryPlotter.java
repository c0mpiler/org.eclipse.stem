...
public class AggregateValueHistoryPlotter extends ReportControl implements ISimulationListenerSync {
	...
	private void createContents() {
		...
		String refFile = Activator.getDefault().getPreferenceStore().getString(PreferenceConstants.REFERENCE_FILE_KEY);
	    ...
	    addReferenceButton.addSelectionListener(new SelectionAdapter() {
			...
			final SelectionEvent e) {
				final FileDialog dd = new FileDialog(
					...
					Activator.getDefault().getPreferenceStore().setValue(PreferenceConstants.REFERENCE_FILE_KEY, selected);
					try {
	...
	public final void setSimulation(final ISimulation simulation) {
		...
		if (this.simulation != null) {
					...
					if (label instanceof DiseaseModelLabel) {
						...
						if (rvp != null) {
							...
							if(totalPopulation.get(populationIdentifier) != null)
								totalPopulation.put(populationIdentifier, Double.valueOf(totalPopulation.get(populationIdentifier).doubleValue()+rvp.getDenominator(null)) );
							else totalPopulation.put(populationIdentifier, Double.valueOf(rvp.getDenominator(null)));
			...
			for(String p:totalPopulation.keySet())
				if(totalPopulation.get(p).doubleValue() <= 0.0) {
					totalPopulation.put(p, Double.valueOf(1.0));
				}
		} else {
			// No
		...
		for (int i = 0; i < timeCount; i ++) {
			if ((valList!=null)&&(i < valList.size()) ){
				...
				if(property.equalsIgnoreCase(REFERENCE_PROPERTY)) {	
					data[i] /= totalPopulation.get(populationId).doubleValue();
				}
			} else {
				// never add zero (need min nonzero value for log display)
				data[i] = 1.0/totalPopulation.get(populationId).doubleValue();
			}
		}
		return data;
	}
	...
	public void sumData() {
		...
		while((iter!=null)&&(iter.hasNext())) {
			...
			if(!dataMap.containsKey(propertyKey)) {
				...
				for (int i = 0; i < timeCount; i ++) {
					newData.add(i,Double.valueOf(0.0));
				}
			...
			totalPopulation.put(populationId,Double.valueOf(totPop));
			aggregateSeriesCanvas.setMinYscale(1.0/totalPopulation.get(populationId).doubleValue());
			sum /= totPop;
			List<Double> aggregateList = dataMap.get(propertyKey);
			// add the new data point
			aggregateList.add(timeCount,Double.valueOf(sum));
