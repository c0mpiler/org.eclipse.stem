...
public class PhaseSpaceCanvas extends Canvas {
		...
		if (rvhp != null) {
			...
			if ((relativeValuesX.length > 0) && ((relativeValuesY.length > 0))) {
				...
				for (int cycleNumber = 0; cycleNumber < relativeValuesX.length; cycleNumber++) {
					...
					if (useLinearScales) {
						valueX = Double.valueOf(displayValueX);
						valueY = Double.valueOf(displayValueY);
					}
					else { 
							valueX = Double.valueOf(Math.log(displayValueX));
							valueY = Double.valueOf(Math.log(displayValueY));
					}
	...
	public final Chart createPhaseSpaceLineChart(
		...
		*/
		...
		final NumberDataSet orthoValuesX = NumberDataSetImpl.create(relativeValuesX);
		final NumberDataSet orthoValuesY = NumberDataSetImpl.create(relativeValuesY);
	...
	protected void resetData() {
		clearData();
		relativeValuesX.add(Double.valueOf(1.0));
		relativeValuesY.add(Double.valueOf(0.0));
	}
