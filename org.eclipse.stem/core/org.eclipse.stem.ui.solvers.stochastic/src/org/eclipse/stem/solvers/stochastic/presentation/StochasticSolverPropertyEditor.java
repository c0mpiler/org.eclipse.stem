...
public class StochasticSolverPropertyEditor extends org.eclipse.stem.core.solver.SolverPropertyEditor {
	 ...
	 */
	public StochasticSolverPropertyEditor(Composite parent,
			...
			if(solver instanceof StandardStochastic) {
				switch(entry.getKey().getFeatureID()) {
					case StochasticPackage.STANDARD_STOCHASTIC__SEED:
						long lVal = (Long.valueOf(entry.getValue().getText())).longValue();
	...
	public boolean validate() {
		...
		// Seed
		text = map
				.get(StochasticPackage.Literals.STANDARD_STOCHASTIC__SEED);
		if (text != null) {
			if (text.getText().equals("")) {
				errorMessage = SolverMessages.getString("SOLV_STANDARD_INVALID_SEED"); //$NON-NLS-1$
				return false;
			} else {
...
//	}
	...
	public Solver createAndPopulateSolver() {
		...
		Solver retValue = StochasticFactory.eINSTANCE.createStandardStochastic();
		
		retValue.setURI(STEMURI.createURI("StandardStochastic"+"/"+STEMURI.generateUniquePart()));
		populate(retValue);
		return retValue;
	}
} // StochasticSolverPropertyEditor
