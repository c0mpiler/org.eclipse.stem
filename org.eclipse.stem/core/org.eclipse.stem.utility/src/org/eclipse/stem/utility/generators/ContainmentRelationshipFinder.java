...
public class ContainmentRelationshipFinder {
		...
		while(iter.hasNext()) {
			...
			if(levelTwoSet.size() >= 1) {
				if (levelOneSet.size()<=0) {
					UtilLogger.logInformation("ERROR. Found Level 2 data with no level 1 data for... "+CNTRY_LVL0_CODE);
					System.exit(1);
				}
				 ...
				 try {
						...
						UtilLogger.logInformation("Error creating file writer " + e.getMessage());
						e.printStackTrace();
						System.exit(1);
					}
				writeL1ToL2Header();
			...
			if (orphanedSet.size() >= 1) {
				...
				UtilLogger.logInformation("Warning, country "+CNTRY_LVL0_CODE+" has "+orphanedSet.size()+" orphaned levelTwoNodes ");
				Iterator ugh = orphanedSet.iterator();
				while(ugh.hasNext()) {
					String code = (String) ugh.next();
					UtilLogger.logInformation("orphan = "+code);
				}// level1
			}
	...
	public static void write(String str) {
		try {
		     fw.write(str+"\n"); 
				...
				UtilLogger.logInformation("Error writing to file writer "+e.getMessage());
			    e.printStackTrace();
			    System.exit(1);
		  }// try
	} // write
	...
	public static void writeL0ToL1Header() {
		...
		write("#/*******************************************************************************");
		write("# * Copyright (c) 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018"); 
		write("# 	* IBM Corporation, BfR, and others."); 
	...
	public static Set getAllCountries() {
		...
		UtilLogger.logInformation("country dir = "+nodeDir);
		File nodes = new File(nodeDir);
		if (nodes.exists()&&nodes.isDirectory()) {
			//UtilLogger.logInformation("found top level data dir");
			...
			for (int i = 0; i < contained.length; i ++) {
				...
				if (file.isDirectory()) {
					...
					if((fileName.length()==3)&&(!excludeSet.contains(fileName))) {
						all.add(fileName);
						icount ++;
						//UtilLogger.logInformation("found "+fileName);
					}// if it's a country dir not on exclude lise
				}// if directory
			}// for all contained files
			UtilLogger.logInformation(" found "+icount+" countries");
		} else {
			UtilLogger.logInformation("top level data dir not found - check static path hardcoded");
		}
	...
	public static Set<String> getNodesInFile(File file ) {
		...
		try {
        	...
        	UtilLogger.logInformation("error opening stream for file "+file.getName());
        	UtilLogger.logInformation(" error "+e.getMessage());
        	e.printStackTrace();
        	System.exit(1);
        }
        ...
        while ( (record=d.readLine()) != null ) { 
           ...
           // UtilLogger.logInformation( "line=  " + record);
        	   ...
        	   int comment = record.indexOf("#");
                if(comment < 0) {
                	...
                	if (eqChr >= 1) {
                		...
                		//************
                		//UtilLogger.logInformation("found key= ["+node+"]");
                		//************
        ...
        d.close();
        	...
        	UtilLogger.logInformation("error reading file "+file.getName());
        	UtilLogger.logInformation(" error "+e.getMessage());
        	e.printStackTrace();
        	System.exit(1);
        }
        ...
        if (!start) {
        	UtilLogger.logInformation(" file "+file.getName()+" has no valid data");
        	System.exit(1);
        }
