...
public class PolygonAreaGenerator {
	...
	protected double getDouble(String str) {
		Double holder = Double.valueOf(str);
				...
				if (reader != null) {
					...
					while (GenUtils.EOF(buffer = reader.readLine()) != true) {
						...
						if (transition == true) {
							...
							// if (oldlevel >= 0) {
							// UtilLogger.logInformation("Sum area for level "
							// + level + " is : " + areaSum);
							// areaSum = 0.0;
							// }
	...
	public static double getPolygonArea(STEMPolygon p) {
		...
		for (int i = 0; i < n - 1; i++) {
			...
			// UtilLogger.logInformation(" ltPoints[" + i + "] = " +
			// ltPoints[i].doubleValue() );
			// UtilLogger.logInformation(" ltPoints[" + (i + 1) + "] = "
			// + ltPoints[i + 1].doubleValue() );
			// UtilLogger.logInformation(" lgPoints[" + i + "] = " +
			// lgPoints[i].doubleValue());
			// UtilLogger.logInformation(" lgPoints[" + (i + 1) + "] = "
		...
		// UtilLogger.logInformation("Area is : " + area);
		area /= 2.0;
	...
	public static void main(String[] args) {
		...
		// UtilLogger.logInformation("\t\t Area data file"); //$NON-NLS-1$
