...
public class PopulationGenerator {
	...
	protected void setURLStatus() {
		...
		urlStatus.put("CIA", Boolean.valueOf(true)); //$NON-NLS-1$
		// All other URLs appear as commented out, secondary.
		urlStatus.put("USCENSUSPOP", Boolean.valueOf(false)); //$NON-NLS-1$
		urlStatus.put("USCENSUSPUERTORICO", Boolean.valueOf(false)); //$NON-NLS-1$
		urlStatus.put("CITYPOP", Boolean.valueOf(false)); //$NON-NLS-1$
		urlStatus.put("CIA", Boolean.valueOf(false)); //$NON-NLS-1$
		urlStatus.put("POP2", Boolean.valueOf(false)); //$NON-NLS-1$
		urlStatus.put("POP", Boolean.valueOf(false)); //$NON-NLS-1$
		urlStatus.put("DEMOGRAFIA", Boolean.valueOf(false)); //$NON-NLS-1$
		urlStatus.put("STATOIDS", Boolean.valueOf(false)); //$NON-NLS-1$
		urlStatus.put("UNITEDNATIONS", Boolean.valueOf(true)); //$NON-NLS-1$
	}
	...
	protected void process() {
			...
			try {
				...
				if (reader != null) {
					...
					while (GenUtils.EOF(buffer = reader.readLine()) != true) {
						...
						// UtilLogger.logInformation("Fix file name is : " + fixFile);
						// // Open the fix writer
						...
						if (transition == true) {
							...
							if (propertiesFile != null) {
								// UtilLogger.logInformation("\t\tGenerated : "
								// + fileName);
					...
					if (propertiesFile != null) {
						propertiesFile.close();
						propertiesFile = null;
						// UtilLogger.logInformation("\t\tGenerated : " + fileName);
						fileName = null;
					}
		...
		// if (fixWriter != null) {
		// UtilLogger.logInformation("Closing fix file");
		// fixWriter.close();
		// fixWriter = null;
		// fixFile = null;
		// }
	...
	protected String getApproximatePopulation(String entity) {
		...
		while (it.hasNext()) {
			...
			if (tokens[LOCATION].equals(entityStr)
				...
				// UtilLogger.logInformation("Value is : " + tokens[1]);
	...
	public static void main(String[] args) {
		...
		// UtilLogger.logInformation("\t\t Population profile"); //$NON-NLS-1$
