...
public class AirTransportGenerator2 {
	...
	public static void main(String [] args) {
		...
		while((line=ireader.readLine()) != null) {
		  ...
		  UtilLogger.logInformation(stemCode+" Population: "+population+ " passengers: "+passengers+" generated: "+generated);
		  while(stemCode != null) {
		...
		for(String stemCode:totalPassengers.keySet()) {
		  ...
		  fw.write(numPassengers+"");
		  fw.write(LS);
		  UtilLogger.logInformation(stemCode+" passengers: "+numPassengers);
		}  // For each line
	...
	public static void writerHeader(FileWriter fw, String filename, String country, int level) throws IOException {
		...
		fw.write("#/*******************************************************************************"+LS); 
		fw.write("# * Copyright (c) 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018"+LS);
		fw.write("# * IBM Corporation, BfR, and others."+LS); 
	...
	public static void readStemPopulationMap(String path) {
		...
		try {
				...
				if (f.getName().endsWith(".properties")) {
					...
					if (fileReader != null) {
						...
						while (EOF(buffer = fileReader.readLine()) != true) {
							...
							// UtilLogger.logInformation(" "+buffer);
							// Tokenize
							// REJECT STRINGS
							...
							if ((buffer.indexOf(s1) == -1)
								...
								// UtilLogger.logInformation(""+stemID2+" = "+population);
								// populationIdMap.put(stemID2, new
								// Integer(population.trim()));
								try {
									Integer popVal = Integer.valueOf(population);
									int itest = popVal.intValue();
									if (itest <= -1) {
										UtilLogger.logInformation("invalid population val for country"
														+ stemID);
										System.exit(-1);
									}
			...
			UtilLogger.logInformation("Error reading file" + useName);
			e.printStackTrace();
			System.exit(1);
		}
	}// readStemPopulation()
