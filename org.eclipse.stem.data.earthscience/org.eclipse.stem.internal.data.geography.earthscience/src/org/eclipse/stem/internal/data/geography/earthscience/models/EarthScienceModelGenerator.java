...
public class EarthScienceModelGenerator extends GeographyModelGenerator {
	...
	public static void main(final String[] args) {
		...
		if (args.length == 0) {
			...
			final String modelPluginId = args[3]; 
			final String modelName = args[4];
			...
			final File geographicModelPluginfile = new File(
					"../../org.eclipse.stem.data/geography/"	+ File.separator + sourceGeographicModelProjecName + File.separator + PluginFileGenerator.PLUGIN_XML_FILE_NAME); //$NON-NLS-1$
			...
			//elevation plugin - used together with all yearly data sets
			final File file2 = new File(
					".."	+ File.separator + "org.eclipse.stem.data.geography.earthscience.elevation" + File.separator + PluginFileGenerator.PLUGIN_XML_FILE_NAME); //$NON-NLS-1$
			final URI geographyPluginfile2 = URI.createFileURI(file2.getAbsolutePath());

			List<URI> geographyPluginFileURIs = new ArrayList<URI>();
			geographyPluginFileURIs.add(geographyPluginfile);
			geographyPluginFileURIs.add(geographyPluginfile2);
			...
			.processFiles(geographicModelPluginFileURI, geographyPluginFileURIs, modelPluginId, modelName);
	...
	protected Map<ISOKey, List<EarthScienceModelSpecification>> processFiles(
			URI geographicModelPluginFileUri, List<URI> geographyPluginFileUris, String modelPluginId, String modelName) {
		...
		for (URI geographyPluginFileUri : geographyPluginFileUris) {
		...
		for (ISOKey isoKey : sortISOKeys(modelMap)) {
			...
			for (AdminLevelSet adminLevelSet : adminSetDCMap.keySet()) {
				...
				if (adminDCMap != null) {
					...
					final EarthScienceModelSpecification temp = new EarthScienceModelSpecification(isoKey, adminLevelSet, countryGeographicModelDC, earthScienceGraphsDC, modelPluginId, modelName);
					gmsList.add(temp);
				}
			} // for adminLevel
